...
@section('content')
    ...
    <div class="flex flex-col justify-center items-center min-h-dvh w-full p-16">
        <div class="flex flex-col justify-center items-center w-1/2">
            {{-- Foto Profile user --}}
            <img src="https://gameranx.com/wp-content/uploads/2023/04/ayakatitel.jpg" alt="logo"
    ...
    <script>
        window.addEventListener('DOMContentLoaded', async () => {
            ...
            const data = await window.axios.get('/api/v1/scans?date=today&per_page=50', {
                headers: {
                    "Accept": "application/json"
                }
            });
            for (let i = 0; i < Math.min(3, data.data.data.length); i++) {
                const scan = data.data.data[i];
